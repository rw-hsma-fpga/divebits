...
\begin{itemize}
	...
	\item Execute the Tcl file \esrpath{\$DIVEBITS/divebits\_demo\_project.tcl} either throug\\\esrmenu{Tools > Run Tcl Script...} or with the console's \esrtype{source} command (replace \$DIVEBITS with the DiveBits installation directory).

	\item The console will show the sequence of required commands, which you can recall anytime with \esrtype{DEMODB\_HELP}.
...
\esrPNG{0.8}{db_swflow_vleft}{DiveBits-enhanced flow}
...
\phantom{X}\\
DiveBits components can be connected in two types of topologies (see Figure~\ref{fig:db_topologies}).
...
\esrPNG{1.0}{db_topologies}{bus topology (left) and daisy-chain topology with CRC (right)}	
	...
	To activate the CRC32 check, select the block IP option \emph{Daisy chain input with CRC check} or set the VHDL generic \esrtype{DB\_DAISY\_CHAIN\_CRC\_CHECK} to \esrtype{true}.
\end{itemize}
...
\begin{itemize}
	...
	For the block design flow, DiveBits can automatically adjust the requisite parameter.
...
\begin{itemize}
	...
	\item \esrtype{READ\_CHECK\_WAIT} reads from the specified \esrtype{ADDR} offset and checks if all bits set in \esrtype{CHECK\_MASK} are identical with the same bits in \esrtype{CHECK\_DATA}. In the example shown above, the master reads from address offset 8 and then only checks the lower 4 bit of that read against 0x3. If they are not identical, it loops and reads again until they are.
\end{itemize}
